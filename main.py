    ...
    if inputFileName:
        if os.path.exists(pathtempAnalysis + inputFileName):
            try:
                ...
                with open(rootDir + pathtempAnalysis + inputFileName, "r") as myfile:
                    for line in myfile:
                        temp = line.split("\t")
                        # creo la stringa per le colonne del dataset
                        if (flag == False):
                            for elem in temp:
                                html += "<th>" + elem.rstrip() + "</th>"
                                html += "</tr></thead><tbody>"
                            flag = True
                        else:
                            html += "<tr>"
                            for elem in temp:
                                html += "<td>" + elem.rstrip() + "</td>"
                                html += "</tr>"
                                html += "</tbody>"
                    return jsonify({'status': 1, 'type': 'success', 'tableHtml': html,
                                    'mess': 'I file output di <b>MITHrIL</b>, sono stati generati con successo!'})
            except:
                return jsonify(
                    {'status': 0, 'type': 'error', 'mess': 'Si è verificato un problema durante l\'esecuzione del jar'})
        else:
            return jsonify({'status': 0, 'type': 'error',
                            'mess': 'Il file di input non è stato trovato, potrebbe non essere stato generato!'})
    else:
